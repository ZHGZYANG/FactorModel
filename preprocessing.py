...
def mkt_data():
  ...
  issue_stats = secd.groupby(['gvkey', 'iid']).apply(lambda x: pd.Series(
    dict(adv=(x['prccd'] * x['cshtrd']).mean(), days=len(x)))).reset_index()
  ...
  mktcap = secd.groupby(['gvkey', 'tic']).apply(lambda x: pd.Series(
    dict(mktcap_pct=(x['prccd'] * x['cshoc']).mean(),adv=(x['prccd'] * x['cshtrd']).mean(),days=len(x)))).reset_index()
       
  secd = pd.merge(secd, mktcap, on=['gvkey', 'tic'], how='left')
  secd.loc[secd['tic'] == "TSLA"]['prccd']
  secd.loc[secd['tic'] == "TSLA"]['ajexdi']
  secd['adj_prccd'] = secd['prccd'] / secd['ajexdi']
  secd.to_pickle(f"{root_fldr}/data/universe_us_raw_hist")
  close_price = secd.sort_values(by=['datadate'])
  def daily_return(group: pd.DataFrame) -> pd.DataFrame:
    group['ret'] = group['prccd'].shift(-1).fillna(method='ffill') / group['prccd'] - 1
    return group
...
def fdmt_data():
  ...
  company = pd.read_pickle(f'{root_fldr}/company.pkl')
  ...
  fundq_raw.to_pickle(f"{root_fldr}/data/est_universe_fundq_us_raw_hist.pkl")